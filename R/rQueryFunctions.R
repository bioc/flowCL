...
#     }
    ...
    if(marker.list[q1]=="CD3" && q3 == 2 ){ # negative marker
        if ( CompInfo == TRUE ) { cat ( "Marker", gsub("CD3e", "CD3", marker.list[q1]), "has been FORCED to update to \"CD3 epsilon\"\n" ) }
...
ontologyLabel <- function ( marker.list, marker.list.short, CompInfo="", save.dir="", que.hasProperLabel="", que.hasProperSynonym="", prefix.info="", endpoint="") {
            ...
            for ( q1 in 1:length ( marker.list[[q3]] ) ) {
                ...
                if(file.exists(fname)){
                    ...
                    if ( length ( temp.location ) == 1 ) {
                        if ( CompInfo == TRUE ) {  cat ( "Marker", gsub("CD3e", "CD3", marker.list[[q3]][q1]), "is called", markers_ShortName_OntologyName[temp.location,2], "\n" ) }
                ...
                if ( nrow ( res ) == 1 ) {
                    ...
                    if ( CompInfo == TRUE ) { cat ( "Marker", gsub("CD3e", "CD3", marker.list[[q3]][q1]), "has been updated to", temp.marker, "\n" ) }
                ...
                if ( nrow ( res ) >= 2 ) {
                    if ( CompInfo == TRUE ) { cat ( "Marker", gsub("CD3e", "CD3", marker.list[[q3]][q1]),"has not been changed, multiple possible markers in label\n" ) }
                }
                if ( nrow ( res ) == 0 | nrow ( res ) >= 2 ) {
                    ...
                    if ( nrow ( res ) >= 1 ) {
                        ...
                        if ( sameLabels == FALSE ) { # label exists however there is two or more labels
                            if ( CompInfo == TRUE ) { cat ( "Marker", gsub("CD3e", "CD3", marker.list[[q3]][q1]), "has not been changed, multiple possible markers in synonyms\n" ) }
                        }
                    }
                    if ( nrow ( res ) == 0 ) { # label does not exist
                        if ( CompInfo == TRUE ) { cat ( "Marker", gsub("CD3e", "CD3", marker.list[[q3]][q1]), "could not be found\n" ) }
                    }
                }
            } # end of for-loop
        } # end of if statement
...
queryMarker <- function ( marker = NULL, query.file = "getMatchingSynonyms.txt",
                        celllabel = NULL, prefix.info="", CompInfo="", endpoint="", exactMatch=FALSE) {
