...
\subsection{Exact-Match with Ontology-Names and Computational Information}
An example of a exact match is shown with ``CD3+CD4+CD8-CCR7-CD45RA+''.
...
The ``CD3+CD4-CD8+CCR7-CD45RA+'' only returned one cell type called ``effector CD8-\\positive, alpha-beta T cell''. The ranking score for this cell type is 1. Therefore, every marker in the query was used in the definition of the cell type. This is a exact match. 
