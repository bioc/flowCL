...
To install {\it \textbf{flowCL}} type {\it source(``http://bioconductor.org/biocLite.R'')} into R and then type \\{\it BiocManager::install(``flowCL'')}. For more information on installation guidelines, see the Bioconductor and the CRAN websites. Once installed, to load the library, type the following into R:
